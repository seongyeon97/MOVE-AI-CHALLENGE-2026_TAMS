...
export function aggregateRange(bundle: DailyBundle, from: string, to: string): Vehicle[] {
  ...
  for (const master of bundle.vehicles) {
    ...
    totalMetrics.set(master.vehicle_id, computeMetrics(split, master.baseline, master.vehicle_class) as unknown as Metrics);
  }
  judge(totalMetrics);
  ...
  for (const month of months) {
    ...
    for (const master of bundle.vehicles) {
      ...
      byVehicle.set(master.vehicle_id, computeMetrics(split, master.baseline, master.vehicle_class) as unknown as Metrics);
    }
    judge(byVehicle);
